...
logger.info(f"GOOGLE_API_KEY: {GOOGLE_API_KEY[:5]}... (hidden for security)")
...
try:
    ...
    raise Exception(f"Failed to initialize Google Forms API: {str(e)}")
...
try:
    ...
    raise Exception(f"Failed to initialize Google Gemini LLM: {str(e)}")
...
def generate_quiz():
    ...
    try:
        ...
        logger.info(f"Quiz generated and stored with ID: {inserted_id}")
...
def create_google_form():
    ...
    try:
        ...
        logger.info(f"Google Form created with ID: {form_id}")
...
def get_latest_form_id():
    """Fetch the latest Google Form ID from MongoDB."""
    try:
        ...
        logger.error(f"Error fetching latest form ID: {str(e)}")
        return jsonify({"error": str(e)}), 500

@app.route('/fetch-responses/<form_id>', methods=['GET'])
def fetch_store_responses(form_id):
    """Fetch and store Google Form responses in MongoDB."""
    try:
        ...
        if user_responses:
            ...
            logger.info(f"Stored {len(user_responses)} responses for form ID: {form_id}")
        ...
        logger.error(f"Error fetching/storing responses: {str(e)}")
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        if form_id:
            try:
                ...
                logger.warning(f"Could not fetch form structure: {str(e)}")
        ...
        logger.info(f"Quiz evaluated for response ID: {user_response_id}, Score: {score}/{total_questions}")
        return jsonify(evaluation_result)

    except Exception as e:
        error_details = traceback.format_exc()
        logger.error(f"Error in evaluate_quiz: {error_details}")
