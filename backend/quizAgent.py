...
def generate_quiz():
    ...
    if content_type == "youtube":
        ...
        try:
            # Use ContextExtractor to extract transcript from YouTube
            content = context_extractor.extract_from_youtube(youtube_url)
            if not content:
                return jsonify({"error": "Failed to extract YouTube transcript"}), 500
                
            # Create a retriever from the extracted content
            text_splitter = RecursiveCharacterTextSplitter(
                chunk_size=2000,
                chunk_overlap=200
            )
            ...
            # Generate quiz using the retriever
    ...
    # Handle file upload (PDF, audio, etc.)
...
@app.route('/fetch-responses', methods=['GET'])
def fetch_store_responses():
    """Fetch responses from Google Forms and store in MongoDB with response time."""
    form_id = "1kHIpMGmx12DrIno24pNPCgDhatKdkZU-yIhKmY78Xcg"  # Replace with your actual Form ID
